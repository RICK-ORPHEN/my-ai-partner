...
export default function HomePage() {
  return (
    <>
      ...
      {/* MOBILE HERO — full-bleed photo + text overlay on cream side */}
      <section className="md:hidden relative overflow-hidden" style={{ background: 'var(--cream)' }}>
        ...
        <div
          className="absolute right-4 bottom-10 z-10 text-cream-50/95 select-none pointer-events-none"
          style={{
        ...
        <div className="relative z-10 px-5 pt-24 pb-14 min-h-[640px] flex flex-col justify-center max-w-[72%]">
          <h1 className="font-serif font-bold leading-[1.2] text-ink jp-balance" style={{ fontSize: 'clamp(1.85rem, 7.4vw, 2.4rem)' }}>
            AIと創る、
            <br />
            <span className="text-vermilion">理想の自分</span>へ。
          </h1>
          <p className="mt-5 text-[12.5px] text-ink jp-text leading-[1.95]">
            AIを活用して、アイデアを形にし、
            <br />
            人生を変えるプロダクトを生み出す。
            <br />
          ...
          <div className="mt-7 flex flex-col gap-3 items-start">
            <Link
              href="/signup"
              className="inline-flex items-center justify-center bg-vermilion hover:bg-vermilion-700 text-cream-50 rounded-xl px-5 py-3 text-sm font-medium tracking-tight transition shadow-lg shadow-vermilion/25"
            >
              無料で始める（クレカ不要）
            </Link>
            <Link
              href="/lesson/restaurant_01"
              className="inline-flex items-center gap-1.5 text-xs text-ink hover:text-vermilion transition border-b border-ink/40 pb-0.5"
            >
              Lesson 1-2 完全無料体験
      ...
      <section className="relative" style={{ background: 'var(--ink)' }}>
        <div className="absolute inset-0">
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(90deg, var(--ink) 0%, var(--ink) 32%, rgba(6,14,49,0.85) 56%, rgba(6,14,49,0.4) 80%, transparent 100%)',
            }}
          />
          <div
            ...
            style={{
              background:
                'linear-gradient(180deg, var(--ink) 0%, rgba(6,14,49,0.92) 22%, rgba(6,14,49,0.5) 42%, rgba(6,14,49,0.5) 58%, rgba(6,14,49,0.92) 78%, var(--ink) 100%)',
            }}
          />
        </div>
        ...
        <div className="md:hidden relative px-5 py-12 text-cream-50">
          ...
          <div className="my-10 flex justify-center">
            <span className="w-12 h-12 grid place-items-center rounded-full border border-cream-50/20">
              <IconArrowRight className="w-5 h-5 text-cream-50" style={{ transform: 'rotate(90deg)' }} />
            </span>
          </div>
          <AfterBlock />
        </div>
      ...
      <section id="works" style={{ background: 'var(--cream)' }}>
        <div className="max-w-[1200px] mx-auto px-5 md:px-10 py-16 md:py-24">
          ...
          <div className="md:hidden space-y-3">
            {ALUMNI.map((a, i) => <AlumniCardMobile key={i} {...a} />)}
          </div>
          <div className="mt-8 md:mt-10 flex justify-center gap-2">
            {[0, 1, 2, 3, 4].map((i) => (
              <span key={i} className={`h-2 rounded-full ${i === 0 ? 'w-6 bg-vermilion' : 'w-2 bg-ink/20'}`} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="relative" style={{ background: 'var(--ink)' }}>
        <div className="absolute inset-0">
          <img src="/images/lp/sunset_cta.png" alt="" aria-hidden className="w-full h-full object-cover opacity-60" />
          <div className="absolute inset-0 hidden md:block" style={{ background: 'linear-gradient(90deg, rgba(6,14,49,0.85) 0%, rgba(6,14,49,0.6) 40%, rgba(6,14,49,0.4) 100%)' }} />
          <div className="absolute inset-0 md:hidden" style={{ background: 'linear-gradient(180deg, rgba(238,236,231,0.4) 0%, rgba(238,236,231,0.15) 25%, rgba(6,14,49,0.4) 60%, rgba(6,14,49,0.85) 100%)' }} />
        </div>

        {/* DESKTOP */}
        <div className="hidden md:grid relative max-w-[1200px] mx-auto px-10 py-28 grid-cols-[1fr,auto] gap-10 items-end">
          <div>
            <h2 className="font-serif font-bold text-cream-50 leading-[1.1] jp-balance" style={{ fontSize: 'clamp(2.4rem, 5vw, 4rem)' }}>
              未来は、創り出せる。
            </h2>
            <p className="mt-5 text-cream-50/85 jp-text text-base md:text-lg leading-[1.85]">
              AIと共に、理想の自分を超えていこう。
            </p>
            <Link href="/signup" className="mt-9 inline-flex items-center justify-center bg-vermilion hover:bg-vermilion-700 text-cream-50 rounded-xl px-7 py-4 font-medium tracking-tight transition">
              無料で始める（クレカ不要）
            </Link>
          </div>
        ...
        {/* MOBILE */}
        <div className="md:hidden relative px-5 pt-14 pb-16">
          <h2 className="font-serif font-bold text-ink leading-[1.15] jp-balance text-center" style={{ fontSize: 'clamp(1.75rem, 7.5vw, 2.3rem)' }}>
            未来は、創り出せる。
          </h2>
          <p className="mt-4 text-ink/80 jp-text text-sm leading-[1.85] text-center">
            AIと共に、理想の自分を超えていこう。
          </p>
          <Link href="/signup" className="mt-7 inline-flex items-center justify-center w-full bg-vermilion hover:bg-vermilion-700 text-cream-50 rounded-xl px-6 py-4 font-medium tracking-tight transition">
            無料で始める（クレカ不要）
          </Link>
          <div className="mt-12 grid grid-cols-3 gap-2">
            <StatLight label="卒業生満足度" value="92" unit="%" />
            <StatLight label="プロダクトリリース率" value="89" unit="%" />
            <StatLight label="学習継続率" value="91" unit="%" />
          </div>
        </div>
      </section>
...
function AfterBlock() {
  return (
    <div>
      ...
      <ul className="space-y-3">
        {['AIを使いこなし、アイデアを形にできる', '自分の強みを活かして価値を届けている', '自由な働き方・理想の人生を実現している'].map((t, i) => (
...
const ALUMNI = [
  { img: '/images/lp/alumni_dashboard.png',  tag: 'SaaS開発・月商300万円',                age: '24歳・フリーランス', text: '自分の業務を効率化するSaaSを開発し、法人向けに提供。' },
  { img: '/images/lp/alumni_chat.png',       tag: 'AIエージェント構築・受託案件獲得',     age: '27歳・会社員',       text: 'AIエージェント開発で受託案件を獲得し、副業で月50万円を達成。' },
  { img: '/images/lp/alumni_education.png',  tag: '教育サービス構築・コミュニティ運営',   age: '26歳・起業家',       text: '自分の経験をコンテンツ化し、教育プラットフォームをリリース。' },
  { img: '/images/lp/alumni_automation.png', tag: '自動化ツール開発・時間と収入の自由',   age: '30歳・マーケター',   text: '業務を自動化するツールを開発し、時間と収入の自由を実現。' },
];
  ...
  return (
    <article className="rounded-2xl overflow-hidden bg-white border border-ink/5 shadow-card">
      <div className="relative aspect-[4/3] bg-cream-50">
        ...
        <div className="absolute top-3 left-3 inline-flex items-center bg-cream-50/95 text-ink text-[10px] font-bold px-2.5 py-1 rounded">{tag}</div>
      </div>
      <div className="p-5">
        <div className="flex items-center gap-2 mb-2">
          <span className="w-7 h-7 rounded-full bg-vermilion/10 text-vermilion grid place-items-center"><IconUser className="w-4 h-4" /></span>
...
function Stat({ label, value, unit }: { label: string; value: string; unit: string }) {
  return (
    <div className="text-center inline-flex flex-col items-center min-w-0">
      <div className="text-[11px] tracking-[0.18em] text-cream-50/85 mb-2 jp-text">{label}</div>
      <div className="relative inline-flex items-end justify-center" style={{ width: 'clamp(140px, 16vw, 180px)', height: 'clamp(118px, 13vw, 150px)' }}>
        <img src="/images/lp/laurel.png" alt="" aria-hidden className="absolute inset-0 w-full h-full object-contain" />
        <div className="relative font-serif font-bold text-cream-50 leading-none pb-1" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)' }}>
          {value}
          <span className="text-base text-cream-50/85 ml-0.5">{unit}</span>
        </div>
      </div>
      <div className="mt-1.5 inline-flex gap-0.5" style={{ color: '#C8A85A' }}>
        {[0,1,2,3,4].map((i) => (
          <svg key={i} viewBox="0 0 20 20" className="w-2.5 h-2.5" fill="currentColor">
...
function StatLight({ label, value, unit }: { label: string; value: string; unit: string }) {
  return (
    <div className="text-center inline-flex flex-col items-center min-w-0">
      <div className="text-[10px] tracking-[0.16em] text-ink mb-1.5 jp-text leading-tight">{label}</div>
      <div className="relative inline-flex items-end justify-center" style={{ width: '110px', height: '92px' }}>
        <img src="/images/lp/laurel.png" alt="" aria-hidden className="absolute inset-0 w-full h-full object-contain" />
        <div className="relative font-serif font-bold text-ink leading-none pb-1" style={{ fontSize: '1.8rem' }}>
          {value}
          <span className="text-sm text-ink-mute ml-0.5">{unit}</span>
        </div>
      </div>
      <div className="mt-1 inline-flex gap-0.5" style={{ color: '#C8A85A' }}>
        {[0,1,2,3,4].map((i) => (
          <svg key={i} viewBox="0 0 20 20" className="w-2 h-2" fill="currentColor">
            <path d="M10 2l2.4 5.4 5.6.5-4.3 3.9 1.3 5.6L10 14.6 4.9 17.4l1.3-5.6L2 7.9l5.6-.5z" />
          </svg>
        ))}
      </div>
